...
- Benchmark results: 1.08 μs p50, 57.3k ops/sec, 4.7× vs Pulse baseline
...
- 11 live markets: BTC, ETH, SOL, AVAX, MATIC, LINK, UNI, ARB, OP, AAVE, DOGE
...
### HFT Nonce Scheme (VEL-10)

High-frequency trading requires sub-microsecond nonce coordination. VEL-10 designs a nonce scheme that:
- Supports concurrent order submission from the same account
- Eliminates the sequential nonce bottleneck for HFT strategies
- Maintains replay prevention guarantees

### CoW Cache Delta Buffer (VEL-11)

Performance optimization for the CoW cache:
- Buffer balance deltas in a compact delta structure
- Coalesce multiple fills per account per batch into a single MPT write
- Reduces MPT write amplification at high fill rates
